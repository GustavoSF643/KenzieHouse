...
def test_if_POST_request_with_a_invalid_json_in_categories_route_return_status_code_NOTACCEPTABLE():
    invalid_dicts = [
        {
            "abc": "a",
            "cba": "b"
    ...
    for status_code in status_code_list:
        assert status_code == 406, "Incorrect status."

def test_if_POST_request_with_a_existent_category_in_categories_route_return_status_code_CONFLICT():
    post_dict = {
        "name": secrets.token_hex(10),
        "description": secrets.token_hex(16)
    }
    ...
    response_data = json.loads(post_response.get_data())
    test_client.delete(f"/categories/{response_data['category_id']}")

    assert response.status_code == 409, "Incorrect status."
